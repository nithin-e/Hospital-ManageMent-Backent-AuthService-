...
import { AuthCallbackResponse, TokenVerificationResponse } from "../../types/authCallbackResponse";
...
export default class AuthController {
 ...
 isAuthenticated = async (call: GrpcCallData, callback: GrpcCallback) => {
  try {
    ...
    if (!token) {
      return callback(null, AuthCallbackResponse.error("Token is mandatory"));
    }
 ...
 verifyToken = async (
  ...
  try {
    ...
    if (!refreshToken || !refreshToken.trim()) {
      return callback(null, TokenVerificationResponse.error("Valid refresh token is required"));
    }
    ...
    if (!refreshTokenSecret || !accessTokenSecret) {
      return callback(null, TokenVerificationResponse.error("Server configuration error"));
    }

    const decoded = jwt.verify(refreshToken, refreshTokenSecret) as DecodedToken;

    if (!decoded?.role || !decoded?.userId) {
      return callback(null, TokenVerificationResponse.error("Invalid refresh token structure"));
    }
    ...
    callback(null, TokenVerificationResponse.error("Token verification failed"));
  }
};
}
